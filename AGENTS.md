...
* [`custom_components/googlefindmy/AGENTS.md`](custom_components/googlefindmy/AGENTS.md#runtime-integration-patterns) — Integration runtime patterns, including the reminder that Home Assistant's forwarding helpers require platform collections to be provided as tuples.
...
* **Dependency bootstrap timing:** Expect `pip install --upgrade homeassistant pytest-homeassistant-custom-component` to run for roughly 3–4 minutes in the hosted environment (downloads + wheel builds). Plan shell work accordingly so lengthy installs finish before kicking off lint or test runs.
