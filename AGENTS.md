...
* **Dev dependency install guard:** Run `make test-deps` after cloning or rebasing to install the full `requirements-dev.txt` stack (and Home Assistant stubs). This avoids skipped or failed `pytest`/`mypy` runs due to missing packages and keeps the resolver checks green without manual pip retries.
