...
- The login container publishes two ports with **different** purposes, and they are configured separately: the one-click token endpoint (7901, read by Home Assistant, published on loopback by default as a security boundary, and widened only through the single `GFMY_ONECLICK_BIND` opt-in, which refuses a wildcard; a widened endpoint carries the bundle in clear text, so it is for a trusted LAN and the seconds of the handoff) and the noVNC viewer (7900, opened by **your** browser, `GFMY_NOVNC_BIND` / `GFMY_NOVNC_URL_HOST`, or simply `bash login.sh --ip <address>`). Home Assistant cannot guess which address your browser can reach, so the config flow only renders a clickable noVNC link for a non-loopback IP. Details, defaults and the network-mode requirement: [`custom_components/googlefindmy/docker-login/README.md`](custom_components/googlefindmy/docker-login/README.md#the-three-address-roles).
