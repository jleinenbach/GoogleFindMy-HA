...
- `make install-ha-stubs` — install `homeassistant` and `pytest-homeassistant-custom-component` into the active environment so `pytest` and the regression helpers work immediately after cloning the repository.
