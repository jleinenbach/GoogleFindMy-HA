...
def test_coverage_threshold_enforced() -> None:
    """The repo must enforce the Platinum quality-scale coverage threshold of 95 %.

    Quality-scale rule ``test-coverage`` requires above 95 % coverage. This test
    asserts that the threshold is wired into the project's coverage configuration
    so it cannot silently drift. Empirical enforcement happens at CI runtime via
    ``--cov-fail-under=95``; this test only verifies the wiring.
    """
    ...
    assert int(fail_under) >= 95, (
        "pyproject.toml fail_under must be at least 95 for Platinum tier"
    )

    ci_match = re.search(r"--cov-fail-under=(\d+)", ci_text)
    assert ci_match, (
        ".github/workflows/ci.yml is missing --cov-fail-under flag in the "
        "pytest invocation (required as a CI-side gate for Platinum)"
    )
    assert int(ci_match.group(1)) >= 95, (
        "CI --cov-fail-under must be at least 95 for Platinum tier"
    )
