...
def test_default_commands_include_ruff_and_pytest(
    ...
    """Ensure the helper runs Ruff format --check before pytest -q."""
    ...
    for exclude in local_verify.DEFAULT_RUFF_EXCLUDES:
        assert f"--exclude={exclude}" in ruff_command

    pytest_command = list(_capture_run[1])
    assert pytest_command[0:2] == ["pytest", "-q"]
...
def test_pytest_still_runs_when_ruff_fails(
    ...
    assert list(recorded[1])[0:2] == ["pytest", "-q"]
