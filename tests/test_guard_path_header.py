...
# existed, so the lenient retrofit posture does not apply to them. The set and
# its two guards exist so the next added module gets pinned instead of
# re-deriving the rule - which is what happened here: review had to point the
# omission out on the module below before it was added to this set.
