...
def _pick_instance_url(hass: HomeAssistant, request: web.Request) -> str:
    """Return the correct base URL matching the current browser origin.

    Rationale (HA best practice):
    - `require_current_request=True` yields the exact scheme/host/port the browser used.
    - If there is no active request context (rare for a view), gracefully fall back to
      an external/hostname-based URL and finally to the default resolution.
    """
    try:
        return get_url(
            hass,
            require_current_request=True,  # match the current browser origin
            allow_internal=True,
            allow_external=True,
            allow_cloud=True,
        )
    except (NoURLAvailableError, HomeAssistantError):
        try:
            # Prefer an external hostname if available and avoid raw IPs
            return get_url(hass, prefer_external=True, allow_ip=False)
        except (NoURLAvailableError, HomeAssistantError):
            # Last resort: whatever HA can provide
            return get_url(hass)
...
class GoogleFindMyMapRedirectView(HomeAssistantView):
    ...
    async def get(self, request: web.Request, device_id: str) -> web.Response:
        """Redirect to the map URL using the same origin as the current request.

        Why this approach?
        - Using HA's `get_url(..., require_current_request=True)` ensures the redirect
          matches the browser's current origin (http/https, host, port), which fixes
          issues with reverse proxies, DuckDNS, and Nabu Casa.
        - Avoids brittle header/IP heuristics and avoids baking local IP/HTTP links.
        """
        auth_token = request.query.get("token")
        if not auth_token:
            return web.Response(text="Missing authentication token", status=400)

        base_url = _pick_instance_url(self.hass, request)

        # Build redirect target (encode query properly)
        from urllib.parse import urlencode

        query = urlencode({"token": auth_token})
        redirect_url = f"{base_url}/api/googlefindmy/map/{device_id}?{query}"
        _LOGGER.debug("Redirecting to: %s", redirect_url)

        # Use an explicit 302 redirect helper from aiohttp
