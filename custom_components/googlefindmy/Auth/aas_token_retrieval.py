...
async def _get_or_generate_android_id(
    ...
    if isinstance(cached_android_id, int):
        android_id = cached_android_id
    elif isinstance(cached_android_id, str):
        try:
            android_id = int(cached_android_id, 0)
        except (TypeError, ValueError):
            _LOGGER.debug("android_id value from cache is not numeric")
