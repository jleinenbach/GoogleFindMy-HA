...
* When annotating Firebase Cloud Messaging helpers, reference the `FcmReceiverHAType` alias exported from `ha_typing`. The concrete runtime implementation loads lazily; using the alias prevents ruff `F821` undefined-name lint failures when the HTTP stack remains optional during import.
