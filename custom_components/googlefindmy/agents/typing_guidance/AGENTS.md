...
* When interpreting Home Assistant registry mappings (for example, `DeviceEntry.config_entries_subentries` inside `services.py`), normalize iterable values to concrete `str` members before using them. Treat lone strings as one-item collections and discard non-string placeholders so strict mypy runs keep accepting tuple or set conversions.
  * **The normalization stays, its meaning does not.** From Core 2026.8 the mapping is a compatibility shim over the scalar fields `DeviceEntry.config_entry_id` and `DeviceEntry.config_subentry_id`, and it always yields exactly one entry with exactly one subentry. Keep the normalizing code for cores at the declared minimum (`2025.9.1`), where the mapping really is many-to-many, but do not read it as set semantics any more: code that branches on "several subentries" is unreachable on a supported modern core, and code that *writes* through the old keywords is wrong there. Both `config_entries` and `config_entries_subentries` are marked for removal in Core 2027.8 (`homeassistant/helpers/device_registry.py` at tag `2026.8.0`, lines 507-508). Prefer the scalar fields where they exist and reach the value through one of the two shared accessors in `coordinator/helpers/registry.py` rather than through `getattr(device, "config_entries", ...)`: `device_belongs_to_entry(device, entry_id)` when the question is whether one entry owns the device, and `device_owning_entry_ids(device)` when the owners themselves are wanted. Both prefer `config_entry_id` and fall back to the shim, so the call site does not branch on the core version. Background: `docs/AI_DEPRECATIONS_GUIDE.md`, section VI.
