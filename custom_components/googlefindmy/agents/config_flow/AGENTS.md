...
* **Manual token UI path disabled.** The `_AUTH_METHOD_INDIVIDUAL` choice in `STEP_USER_DATA_SCHEMA` remains commented out because the manual token + email path is broken. Keep the commented line (with its inline note) intact until the workflow is fixed and ready to re-enable. The reauth form’s manual token field is similarly commented out with a matching note so the broken path stays hidden there as well. The options flow credential refresher (`async_step_credentials`) also keeps the manual token input commented out.
  * **Approved exception: container login.** Besides `secrets.json` uploads, the existing-entry forms (`async_step_reauth_confirm` and `async_step_credentials`) may expose the one-click **container login** fields (`container_host`, `container_port`, `pairing_code`). This is an explicit, narrow carve-out from the "secrets-only" wording above, which was written against the *broken manual token path*, not as a ban on future working methods. The exception holds only while the container path keeps its own end-to-end coverage (`tests/test_config_flow_container_login.py`, `tests/test_token_server.py`) and its own error keys (`container_unreachable`, `container_timeout`, `container_auth_failed`, `container_code_used`, `container_login_failed`); it does not license re-enabling the manual token input or adding further methods without a matching amendment here.
    * **Error-key list corrected against the code (PR #1211).** The list above previously named `invalid_pairing_code`, which `grep` finds **nowhere** in `config_flow.py` or `strings.json`, while the two keys `_map_container_error` really produces for a spent code and a generic failure (`container_code_used`, `container_login_failed`) were missing. Both directions of the drift are now closed. Keep them closed the same way: every key listed here must have at least one hit in `config_flow.py`, and every key `_map_container_error` returns must appear here. Check both directions, or the drift simply reappears the other way round.
  * **Amendment: the container login is a two-step flow.** `async_step_container_login` collects the container *address* only; `async_step_container_pairing` collects the pairing code. The split is not cosmetic: the container prints that code only after the interactive Google sign-in has finished, i.e. strictly after the user needed the address to reach the session, so a single combined form demanded a value that could not exist yet. `host` and `port` therefore live in the flow state between the two steps, and a detour back to the address step (which is what `container_unreachable` is: an address problem, not a code problem) must preserve an already-typed code.
  ...
  * **Exactly one method per submission.** Because the container `GET` is one-shot, a submission that carries more than one credential method must fail with `choose_one` *before* any network call, so pasted credentials are never silently discarded and no pairing code is consumed. Route the check through the shared helper rather than re-deriving the precedence at each call site (`async_step_reauth_confirm` and `OptionsFlowHandler.async_step_credentials` share it).
