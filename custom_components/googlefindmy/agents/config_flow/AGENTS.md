...
* **The service and hub groups never carry `visible_device_ids`, and a device target is decided by a predicate, not by a bare key comparison.** Three production sites already write the invariant that way: `ServiceSubentryFlowHandler._visible_device_ids` and `HubSubentryFlowHandler._visible_device_ids` both return `()` unconditionally (unlike the `_BaseSubentryFlow` version they override, which reads the stored value), and `_async_sync_feature_subentries` builds its `service_payload` through `_build_subentry_payload` **without** the `visible_device_ids` argument while `tracker_payload` passes the stored ids. `tests/test_config_flow_subentry_sync.py` pins that result. The reading side is a fourth, separately tracked enforcement (see below), and `ConfigEntrySubEntryManager.update_visible_device_ids` a fifth; a test asserts, it does not write. None of the three was an authoritative checkpoint, which is why `_async_assign_devices_to_subentry` could store device ids on the service group anyway. It now refuses such a target, and the three steps that assign devices (`async_step_visibility`, `async_step_repairs_move`, and the `_FIELD_REPAIR_FALLBACK` field of `async_step_repairs_delete`) build their choices from `_device_target_choice_map` instead of `_subentry_choice_map`.
  ...
  * **A second writer inside this module is still unguarded: `_async_sync_feature_subentries`.** Its `service_payload` omission (above) protects the *payload*, not the *target*. Its local `_resolve_existing` has no type axis of its own, and the seeding in front of it does not supply one. Its fallback scan matches on the stored `group_key`; its first branch trusts whatever the seeded context map holds, which is empty on the migration path (`_ensure_subentry_context`), while on the reconfigure path `_reset_reconfigure_subentry_context` canonicalises by type in its first loop but then re-adds by stored key in its second, so a **`service`-typed** twin ends up seeded under both keys and is written twice: first with the id-less `service_payload`, then with `tracker_payload`. A `hub`-typed one is seeded under the tracker key only, because the manager canonicalises `service` and `tracker` by type but leaves `hub` on its stored key, and it is written once, straight with `tracker_payload`. Measured on both paths and for both types, not inferred from the seeding code. A `service`- or `hub`-typed subentry that still stores `core_tracking` is therefore resolved *as* the tracker group and receives `tracker_payload`, and `_async_update_subentry` does not change `subentry_type`, so the invariant breaks on a stored subentry. The `if not tracker_visible and self._available_devices` fallback makes that worse than a pass-through: it fills a previously clean twin with every probed device id. Measured against the real helper rather than argued: both a `service`- and a `hub`-typed twin storing `core_tracking` come back with `visible_device_ids` set. The mirror direction breaks too, and it loses data instead of adding it: a `tracker`-typed subentry storing the *service* key is resolved as the service group, receives the id-less `service_payload`, and the update drops the assignment it held. Neither direction is hypothetical — `tests/test_coordinator_subentry_visibility.py` pins the same legacy shape on the reading side — and no test covers either one on the writing side. Closing it needs a type axis in that resolver, which turns an update into a create on the migration and reconfigure paths; that is left to its own change rather than folded in here, so do not read this paragraph as describing a guarded state.
