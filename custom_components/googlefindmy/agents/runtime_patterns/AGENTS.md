...
* **Platform forwarding reminder:**
  ...
  * Home Assistant 2025.11 removed the singular `async_forward_entry_setup`. Subentries must now rely on platform-level `config_subentry_id` routing: `_async_ensure_subentries_are_setup` only yields to the loop and logs the absence once. Do **not** attempt to resurrect the singular helper.
  * Ensure every platform validates that a sanitized `config_subentry_id` is available before creating entities or devices; skip creation (with debug logging) otherwise so registries never contain orphaned entries.
