...
* **Platform forwarding reminder:**
  ...
  * **Never** fall back to the plural helpers for subentries, even when older Home Assistant builds lack the singular API. Those cores are not supported for config subentries; falling back silently reintroduces the `config_subentry_id=None` bug documented in the handbook's postmortem.
  ...
  * The `RuntimeData.legacy_forwarded_platforms`/`legacy_forward_notice` fields only activate when Home Assistant exposes `async_forward_entry_setup` **without** `config_subentry_id`. The weakref-backed tracker logs a single warning for these unsupported cores while remembering the platforms forwarded without identifiers, so future retries can avoid spamming the log. Leave the fields untouched for modern installs.
