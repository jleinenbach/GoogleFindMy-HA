...
* **Platform forwarding reminder:**
  * The parent `async_setup_entry` handles coordinator/bootstrap work **and subentry creation only**. After creating child entries, return `True`—Home Assistant will automatically call each platform's `async_setup_entry` with the correct `config_subentry_id` for every subentry.
