...
def _core_auto_schedules_subentries(config_entries: Any) -> bool:
    ...
    return bool(
        ...
        and callable(getattr(config_entries, "async_schedule_entry_setup", None))
    )
    ...
    *,
    ...
    if skip_registered and registered_all:
        if auto_schedules_subentries:
            _LOGGER.debug(
                "[%s] Home Assistant advertises registered subentries; forwarding anyway to guarantee platform load",  # noqa: G004
                parent_entry.entry_id,
            )
        else:
...
async def async_setup_entry(hass: HomeAssistant, entry: MyConfigEntry) -> bool:
    ...
    config_entries = getattr(hass, "config_entries", None)
    has_registered_subentries = isinstance(entry.subentries, Mapping) and bool(
        entry.subentries
    )
    ...
    elif has_registered_subentries:
        suppress_subentry_forwarding = True
        _LOGGER.debug(
            "[%s] Suppressing manual subentry forwarding during setup; %s registered subentries already present",
            entry.entry_id,
    ...
    if registered_subentries:
        _LOGGER.debug(
            "[%s] Subentry forward suppressed; %s registered subentries will be scheduled by Home Assistant%s",
            entry.entry_id,
            len(registered_subentries),
            " (auto-scheduling detected)" if auto_schedule_subentries else "",
        )
