    ...
    if isinstance(resolver_map, dict):
        for resolver in resolver_map.values():
            ...
            if isinstance(resolved_entry_id, str) and resolved_entry_id in receivers:
                bucket["default_fcm_entry_id"] = resolved_entry_id
                return receivers[resolved_entry_id]

    default_entry_id = bucket.get("default_fcm_entry_id")
    if isinstance(default_entry_id, str) and default_entry_id in receivers:
        return receivers[default_entry_id]
...
async def _async_acquire_shared_fcm(
    ...
    *,
    ...
    async with fcm_lock:
        ...
        if receivers and fcm is None:
            _LOGGER.warning(
                "Discarding cached FCM receiver with unexpected type",
            )
            stale = _pop_any_fcm_receiver(bucket)
            await _async_stop_receiver_if_possible(stale)
            fcm = None
