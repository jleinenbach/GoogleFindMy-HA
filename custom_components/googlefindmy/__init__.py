...
async def _async_unload_parent_entry(hass: HomeAssistant, entry: MyConfigEntry) -> bool:
    ...
    if callable(unload_callable) and not already_unloaded:
        try:
            ...
            setattr(entry, "_gfm_parent_platforms_unloaded", unload_parent_platforms)
    else:
        if not callable(unload_callable):
            _LOGGER.debug(
            ...
            _LOGGER.debug(
                "[%s] Parent platforms already marked as unloaded; skipping duplicate call",
                entry.entry_id,
            )
