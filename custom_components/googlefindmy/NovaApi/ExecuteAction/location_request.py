...
async def get_location_data_for_device(
    ...
    *,
    ...
    try:
        ...
        # Use explicitly passed cache for multi-account support instead of ContextVar
        # This prevents cross-contamination when using run_coroutine_threadsafe
        cache_provider = None
        if cache:
            # Capture the cache instance directly (not via ContextVar)
