    ...
    async def _async_update_data(self):
        ...
        try:
            ...
            if should_poll_location and devices:
                _LOGGER.info(f"Polling location for {len(devices)} devices")
                
                # Poll all devices in parallel (but with small delays to avoid overwhelming the API)
                for i, device in enumerate(devices):
                    ...
                    try:
                        ...
                        _LOGGER.info(f"Requesting location for {device_name}")
                        ...
                        if location_data:
                            ...
                            if lat is not None and lon is not None and -90 <= lat <= 90 and -180 <= lon <= 180:
                                if last_seen > 0:
                                    ...
                                    _LOGGER.info(f"Got location for {device_name}: lat={lat}, lon={lon}, accuracy={acc}m, age={location_age_hours:.1f}h")
                                else:
                                    _LOGGER.info(f"Got location for {device_name}: lat={lat}, lon={lon}, accuracy={acc}m")
