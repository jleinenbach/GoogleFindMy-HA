...
Summarize cached wheels and validate manifest coverage. This helper inspects a
local wheelhouse directory (defaulting to ``.wheelhouse/ssot``), prints a
grouped summary of the generated wheels, and optionally validates the cache
against a manifest of ``fnmatch`` patterns. Reviewers without an on-disk cache
can pass ``--allow-missing`` to preview the formatter without needing to
...
def report_manifest_coverage(
    patterns: Sequence[str], records: Sequence[WheelRecord]
) -> int:
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Summarize cached wheelhouse contents."
    )
    parser.add_argument(
        "wheelhouse",
        nargs="?",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional manifest of glob patterns to validate against the wheel cache."
        ),
    )
    parser.add_argument(
        "--allow-missing",
        action="store_true",
        help=(
            "Continue even if the wheelhouse directory is absent (prints an "
            "empty summary)."
        ),
    )
    return parser
...
def main() -> int:
    ...
    if not wheelhouse.exists():
        ...
        print(
            ...
            "No wheel cache found; run without --allow-missing after caching "
            "wheels to validate contents.",
            sep="",
        )
